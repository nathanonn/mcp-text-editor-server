...
async function createTextEditorServer(baseDir: string) {
    ...
    server.tool(
        ...
        "View file content or directory listing",
        {
            path: z
                .string()
                .describe(
                    "File or directory path relative to the base directory"
                ),
            view_range: z
                .array(z.number())
                .optional()
                .describe(
                    "Optional range of lines to view [start, end]. Line numbers are 1-indexed and -1 for end means read to end of file."
                ),
        },
                ...
                if (stats.isDirectory()) {
                    ...
                    if (view_range && view_range.length === 2) {
                        ...
                        const endLine =
                            view_range[1] === -1
                                ? lines.length - 1
                                : Math.min(lines.length - 1, view_range[1] - 1);
    ...
    server.tool(
        ...
        "Replace text in a file",
        {
            path: z
                .string()
                .describe("File path relative to the base directory"),
            ...
            count: z
                ...
                .optional()
                .describe(
                    "Number of occurrences to replace (all if not specified)"
                ),
        },
            ...
            try {
                ...
                if (count !== undefined && count > 0) {
                    ...
                    content = content.replace(
                        new RegExp(escapeRegExp(old_str), "g"),
                        (match) => {
    ...
    server.tool(
        ...
        "Insert content at a specific line number",
        {
            ...
            line_number: z
                .number()
                .describe("Line number to insert at (1-based)"),
        ...
        async ({ path: filePath, content, overwrite }) => {
            try {
                ...
                if (fsSync.existsSync(resolvedPath) && !overwrite) {
                    throw new Error(
                        `File already exists: ${filePath}. Use overwrite=true to replace it.`
                    );
                }
